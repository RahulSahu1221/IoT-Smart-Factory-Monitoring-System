...
{
  //read value from, check if it is true for next 10 secs
...
static uint16_t readLDR (void)
{
  ...
  } return sum/4;   //if warehouse is dark : value<300 = dark, valuse> 300 = light
}



//read data from sensor, return status of DHT22 ->  returns 0 on error
bool sensor_read(SensorData* out, SensorManager* sm)
{
  ...
  //read ldr //read door status // read motion detected
